...
@Service
public class MonitorCenterListener {
	...
	@Autowired
	private MonitorCenterService service;
	
	@Bean("monitorCenterContainerFactory")
	public ConcurrentKafkaListenerContainerFactory<Integer, String> listenerContainer() throws NemsException {
		...
		container.setConcurrency(calcTopoService.getConcurrency(CalcTopoService.CLIENT_ID_MONITOR_CENTER));
		container.setBatchListener(true);
		
		return container;
	}

	@KafkaListener(id = CalcTopoService.CLIENT_ID_MONITOR_CENTER+"-${cie.app.id}", 
			clientIdPrefix = CalcTopoService.CLIENT_ID_MONITOR_CENTER+"-${cie.app.id}", 
			topics = {"${monitor-center-topic-name}" }, 
			groupId = CalcTopoService.CLIENT_ID_MONITOR_CENTER, 
			containerFactory = "monitorCenterContainerFactory")
