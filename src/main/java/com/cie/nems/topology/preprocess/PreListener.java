...
public class PreListener {
	...
	public ConcurrentKafkaListenerContainerFactory<Integer, String> listenerContainer() throws NemsException {
		...
		container.setConcurrency(calcTopoService.getConcurrency(CalcTopoService.CLIENT_ID_PRE));
		container.setBatchListener(true);
		
		return container;
	}

	@KafkaListener(id = CalcTopoService.CLIENT_ID_PRE+"-${cie.app.id}", 
			clientIdPrefix = CalcTopoService.CLIENT_ID_PRE+"-${cie.app.id}", 
			topics = {"${pre-topic-name}" }, 
			groupId = CalcTopoService.CLIENT_ID_PRE, 
			containerFactory = "preContainerFactory")
