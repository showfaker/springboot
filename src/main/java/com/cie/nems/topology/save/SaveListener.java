...
public class SaveListener {
	...
	public ConcurrentKafkaListenerContainerFactory<Integer, String> listenerContainer() throws NemsException {
		...
		container.setConsumerFactory(new DefaultKafkaConsumerFactory<>(kafkaConfig.batchConsumerProps()));
		container.setAutoStartup(false);
		container.setConcurrency(calcTopoService.getConcurrency(CalcTopoService.CLIENT_ID_SAVE));
		container.setBatchListener(true);
		
		return container;
	}

	@KafkaListener(id = CalcTopoService.CLIENT_ID_SAVE+"-${cie.app.id}", 
			clientIdPrefix = CalcTopoService.CLIENT_ID_SAVE+"-${cie.app.id}", 
			topics = {"${save-topic-name}" }, 
			groupId = CalcTopoService.CLIENT_ID_SAVE, 
			containerFactory = "saveContainerFactory")
