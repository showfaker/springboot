...
public class AlarmListener {
	...
	public ConcurrentKafkaListenerContainerFactory<Integer, String> listenerContainer() throws NemsException {
		...
		container.setConcurrency(calcTopoService.getConcurrency(CalcTopoService.CLIENT_ID_ALARM));
		container.setBatchListener(true);
		
		return container;
	}

	@KafkaListener(id = CalcTopoService.CLIENT_ID_ALARM+"-${cie.app.id}", 
			clientIdPrefix = CalcTopoService.CLIENT_ID_ALARM+"-${cie.app.id}", 
			topics = {"${alarm-topic-name}" }, 
			groupId = CalcTopoService.CLIENT_ID_ALARM, 
			containerFactory = "alarmContainerFactory")
